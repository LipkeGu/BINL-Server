...
int Connection::CreateUDPSocket(int Broadcast, int ReUseAddr, uint16_t port)
{
	...
	if (this->Conn != SOCKET_ERROR)
	{
		if (this->serverType == DHCP)
		{
			...
			this->SocketState = setsockopt(this->Conn, SOL_SOCKET, SO_BROADCAST, (const char*)&Broadcast, sizeof(Broadcast));
		...
		if (this->serverType == HTTP)
		{
			...
			this->SocketState = setsockopt(this->Conn, SOL_SOCKET,
				SO_KEEPALIVE, (const char*)&ka, sizeof(ka));
...
int Connection::Listen()
{
	...
	while (this->SocketState != SOCKET_ERROR)
	{
		...
		if (this->serverType != HTTP)
			this->SocketState = recvfrom(this->Conn, packet->GetBuffer(), packet->GetLength(),
...
#ifdef _WIN32
			this->SocketState = read(*s, packet->GetBuffer(), packet->GetLength(), 0);
#else
...
int Connection::Send(Client* client)
{
	if (this->serverType == DHCP)
	{
		...
		this->SocketState = sendto(this->Conn, client->Data->GetBuffer(), client->Data->GetPosition(), 0,
		...
		else
		{
			...
			do
			{
				bs += send(*client->GetSocket(), &client->Data->GetBuffer()[bs], client->Data->GetPosition() - bs, 0);
